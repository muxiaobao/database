...
typedef struct {
    uint32_t id;
    char username[COLUMN_USERNAME_SIZE + 1]; // C strings end with a null character
...
// this macro function gets the length of the attribute in the struct
...
// A cursor represents a position in the table
// In a tree,  we identify a position by the page number of the node and the cell number within that node
typedef struct {
    Table* table;
    uint32_t page_num;
    uint32_t cell_num;
...
uint32_t* internal_node_key(void* node, uint32_t key_num) {
    return (void*)internal_node_cell(node, key_num) + INTERNAL_NODE_CHILD_SIZE; // pointer arithmetic (if no type cast, will add 4 * sizeof(uint32_t))
}
...
// Every node takes up one page, no need to deal with partial pages
...
// Free the memory for the Pager and Table data structures
...
+           TABLE_MAX_PAGES);
    ...
    if (pager->pages[page_num] == NULL) {
        ...
        // if necesary, save a partial page at the end of the file
        if (pager->file_length % PAGE_SIZE) {
            num_pages += 1;
        }

        if (page_num <= num_pages) {
            // relocate file descriptor to the beginning of the page whose number is page_num
            lseek(pager->file_descriptor, page_num * PAGE_SIZE, SEEK_SET);
            // read into the buffer pointed by 'page'
...
// parse row to generate its compact representation
void serialize_row(Row* source, void* destination) {
    memcpy(destination + ID_OFFSET, &(source->id), ID_SIZE);
    memcpy(destination + USERNAME_OFFSET, &(source->username), USERNAME_SIZE);
    memcpy(destination + EMAIL_OFFSET, &(source->email), EMAIL_SIZE);
}
// generate a row from its compact representation
...
/* Until we start recycling free pages, 
   new pages will always go onto the end of the database file (0-based)
*/
...
/* Take the right child as input and allocate a new page to store the left child */
...
void internal_node_insert(Table* table, uint32_t parent_page_num, uint32_t child_page_num) {
    ...
    *internal_node_num_keys(parent) = original_num_keys + 1;
    ...
    void* right_child = get_page(table->pager, right_child_page_num);
    if (child_max_key > get_node_max_key(right_child)) { // case: split occurs in the rightmost leaf node
        /* Replace right child */
  ...
  for (int32_t i = LEAF_NODE_MAX_CELLS; i >= 0; i--) {
    ...
    *(leaf_node_num_cells(new_node)) = LEAF_NODE_RIGHT_SPLIT_COUNT;
    ...
    if (is_node_root(old_node)) {
        ...
        uint32_t new_max = get_node_max_key(old_node); // maximum key of the new left node (namely, old_node)
        void* parent = get_page(cursor->table->pager, parent_page_num);
        /* Update parent node with new_max */
...
ExecuteResult execute_insert(Statement* Statement, Table* table) {
    ...
    // insert into the lead node at the position returned by table_end(), rows still unsorted
